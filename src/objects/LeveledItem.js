const extractDices = (item) => {
  const lv = item.level || 10;
  const packedDices = [...item.stats, ...item.statGrow].filter(
    (dice) => dice.item === "dice"
  );
  let leveledDices = [];
  let nothing = packedDices.map((dice) => {
    ...
    if (!!dice.levelMakesExtraDice && dice.amountOfLevelsForExtraDice) {
      for (
        let i = Math.floor(lv / dice.amountOfLevelsForExtraDice);
        i > 0;
        i--
      ) {
        leveledDices.push({ ...tempDice });
...
export const LeveledItem = {
  ...
  initiate(data) {
    this.dices = extractDices(data);
    this.name = data.name;
    this.alt = data.alt;
    this.skill = {
      skillName: data.skill.skillName,
      skillTxT: data.skill.skillTxT,
      id: data.skill.id,
    };
    this.statsTxT = data.statsTxT;
    this.lv = data.lv;
    console.log(
      `Initiating item --- ${this.name} Lv${this.lv} ${
